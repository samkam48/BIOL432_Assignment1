...
MData <- NewMeasurements %>% 
  ...
  mutate(FirstLetter=gsub("^([A-Za-z]).*", "\\1.", Organism)) %>% 
  mutate(LastWord = gsub(".*^([A-Za-z])+.", "", Organism)) %>% 
  unite(Sp, FirstLetter, LastWord, sep = "", remove = FALSE) %>% 
  select(Organism, Limb_width, Limb_length, Observer, Limb_volume, Sp)
```
